...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    if !args.contains(&String::from("-m")) && !args.contains(&String::from("--message")) {
        let prompt = format!("This is a diff from a recent change of a library. Explain what was changed as concise as possible.\n{}", diff);
        ...
        if let Some(msg) = Editor::new()
            .require_save(false)
            .extension("txt")
            .edit(&items[selection].replace('"', "\\\""))
            .unwrap()
        {
